...
onPageScroll(e) {
  ...
  console.log("=====onPageScroll======"+e.nativeEvent.offset);
}
  ...
  return (
    <View style={styles.container}>
      <ViewPager
       ...
       renderPage={(data, page)=>this._renderPage(data,page)}
...
_renderPage(data: Object, page: number | string,) {
    if(page == 0){
      return (
        <View style={styles.page}>
          <Top_view />
  ...
  _onChangePage(pageId: number | string){
   this.bottomView.setChange(pageId);
    this.setState({
      page:pageId,
    });
  }
   ...
   renderV(){
     return(
       <View style={styles.container}>
         ...
         <Bottom_view style={styles.bottom_view_style}/>
       </View>

     );
   }

  renderListview(itemData){
     return (
        <TouchableOpacity activeOpacity={1} onPress={()=>this.onClickListViewItem(itemData)}>
          <Live_item
          listSource={itemData}
